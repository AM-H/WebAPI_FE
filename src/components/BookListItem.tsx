  ...
  return (
    <Link href={`/books/${book.isbn13}`} passHref legacyBehavior>
      <ListItem button component="a">
        <ListItemAvatar>
          <Avatar src={cover} alt={book.title} variant="square" sx={{ width: 56, height: 80, marginRight: 2 }} />
        </ListItemAvatar>
        <ListItemText primary={book.title} secondary={`${authors} • ${year}`} secondaryTypographyProps={{ color: 'gray' }} />
      </ListItem>
    </Link>
  );
}
